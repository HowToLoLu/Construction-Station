...
/datum/mutation/strongwings/on_acquiring()
	if(..())
		return
	var/obj/item/organ/wings/wings = owner.getorganslot(ORGAN_SLOT_EXTERNAL_WINGS)
	if(!wings)
...
/datum/mutation/strongwings/modify()
	..()
	var/obj/item/organ/wings/bee/bee_wings = owner.getorganslot(ORGAN_SLOT_EXTERNAL_WINGS)
	if(istype(bee_wings))
		bee_wings.jumpdist = initial(bee_wings.jumpdist) + (6 * GET_MUTATION_POWER(src)) - 3
